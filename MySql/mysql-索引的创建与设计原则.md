...
+ 在声明有`主键约束, 唯一性约束, 外键约束`的字段上,会自动的添加对应的`主键索引,唯一性索引,普通索引`

+ 创建唯一索引,会自动对相关字段(联合字段)添加唯一性约束(如果之前没有)
...
会自动添加主键索引: emp_id,  唯一性索引: emp_name, 普通索引: emp_dept_id_fk
...
- `INDEX`与`KEY`为同义词，两者的作用相同，用来指定创建索引, 一般使用`INDEX`；
...
在声明有`主键约束`的字段上,会自动的添加`主键索引`, `主键索引`也只能通过定义`主键约束`的方式添加
